---
title: "Contracts and CRE"
description: "Smart contracts, Chainlink CRE workflows, and AI evaluation architecture."
---
...
```mermaid
flowchart TD
    A[Host creates listing] -->|On-chain| B[KXManager.sol]
    C[Attendee requests session] -->|On-chain| B
    ...
    G -->|HTTP Write| J[IPFS]
    I -->|Claim payouts| K[Host/Attendee claims]
```
...
sequenceDiagram
    ...
    participant IPFS as IPFS Store

    Host->>KXManager: Create listing (topic, price, dataCID)
    Attendee->>KXManager: Request session for listing (meeting link)
    ...
    Note over Attendee,Host: Session occurs off-chain (e.g., video call)
    Host->>SettleWF: Request evaluation (transcript dataCID)
    ...
    SettleWF->>IPFS: Write settlement data
    Host->>Registry: Claim host payout
    Attendee->>Registry: Claim attendee refund (if applicable)
```
...
```mermaid
flowchart LR
    KXManager[KXManager] -->|sessionRegistry| KXRegistry[KXSessionRegistry]
    KXManager -->|usdc| USDC[USDC Token]
    KXRegistry -->|holds escrow| USDC
    KXManager -->|emits event| InitWF[Initiation WF]
    InitWF -->|registers| KXRegistry
    KXRegistry -->|emits event| SettleWF[Settlement WF]
    SettleWF -->|onReport| KXRegistry
```
...
stateDiagram-v2
    [*] --> None
    None --> Funded: Attendee pays USDC
    Funded --> Evaluated: CRE returns AI scores
    Evaluated --> Finalized: Dispute window passes
    Evaluated --> Disputed: Participant disputes
    Disputed --> Evaluated: Dispute resolved
    Finalized --> [*]: Claim complete
```
...
flowchart LR
    ...
    subgraph calc [Calculation]
        Eff[effective = avg]
        Sq[teacherShareBps = effective²]
        Amt[teacherAmount = amount × share]
    end
    Conf --> Eff
    Learn --> Eff
    Eff --> Sq
...
```mermaid
flowchart TB
    subgraph init [Initiation Workflow]
        InitWF[Initiation WF]
    end
    subgraph settle [Settlement Workflow]
        SettleWF[Settlement WF]
    end

    InitWF -->|POST /bot| Recall1[Recall.ai]
    InitWF -->|Write| Pinata1[Pinata IPFS]
    SettleWF -->|GET /transcript| Recall2[Recall.ai]
    SettleWF -->|POST chat completions| Gemini[Gemini AI]
    SettleWF -->|Write| Pinata2[Pinata IPFS]
```
...
For full setup, environment variables, and Abi Ninja testing, see [Testing CRE Workflows](/cre-testing).
